...
`ok: false` comes with a `problems` array that names what is wrong. If it says
`ok: true`, the server is fine and the problem is between the contract, the
marketplace, and a cache.
...
token. That is how a pre-reveal drop works, and it is the single most common
mistake here.
...
check `/status`: if `mintState.lastError` is set, the RPC endpoint is failing, and
the server is failing closed on purpose. A stale `highestMintedTokenId` with no
error usually means `tokenIdStart` is wrong, so every token is off by one.

`metadata-missing` means the server sees the mint but has nothing to serve. Either
the manifest is smaller than the drop, or the shuffle is on and `SHUFFLE_SEED` is
not set. `/status` says which.
...
If the header is right but placeholders still persist, look for a cache rule you
added in front of the server: a Cloudflare cache rule, a CDN in front of Vercel,
or a "cache everything" page rule. Those override the origin.
...
contract address, which is deliberate: a server that answered requests while
pointed at `0x000...` would be worse than one that refuses to start.

## Reveals are slower than ten seconds

`mintState.ttlSeconds` is the floor on how late a reveal can be, but a serverless
host runs many instances and each keeps its own timer, so a request landing on a
cold instance waits for a fresh read. Options, in order of effort: lower
`ttlSeconds` to 5, add a webhook, or add a webhook plus a KV store so instances
share progress. See [webhooks.md](webhooks.md).
...
delivery claimed it did. Check `mintState.webhookMints` at `/status`. A high water
mark only rises, so restarting the server is the only way back down.

## Rate limited by the RPC endpoint

You are on a shared public endpoint. Set `RPC_URL` to your own. Any provider works.

In `sequential` mode the call rate does not depend on traffic, so if you are being
limited at all, it is either the shared endpoint or `ownerOf` mode on a heavily
indexed drop.
...
The signature did not match, which means the signing key is not the one for this
...
prints the range it accepted. If that range disagrees with the contract, preflight
will say so:
...
along with the `x-reveal-state` and `cache-control` headers for one token that is
behaving wrongly, and what `cast call <contract> "tokenURI(uint256)(string)" <id>`
returns for it.
